...
class ProtOpenMMSystemSimulation(EMProtocol):
    ...
    User IA Manual: SystemSimulation Protocol

The SystemSimulation protocol enables the execution of molecular dynamics
simulations using OpenMM on a system that has been previously prepared. It
takes as input the full topology and coordinates of a molecular system, and
performs one or more simulation stages depending on the user?s configuration.
This protocol supports both minimization and production dynamics, and can be
used for equilibrium analysis, conformational sampling, or stability assessment
of biomolecular complexes.

To begin, the user must provide the prepared system files generated by a prior
step, such as SystemPrep. These files include the molecular topology, atomic
coordinates, box dimensions, and force field definitions, all of which are
required to launch the simulation correctly. The user also specifies how long
the simulation should run, defined in terms of the number of steps or total
simulation time. The time step for integration is configurable and typically
depends on whether constraints are used on bond lengths involving hydrogens.

The user may choose to perform an initial energy minimization to relax the
system geometry before beginning the simulation. This step helps remove any
residual strain or steric clashes that could cause instability during
dynamics. The temperature, pressure, and integrator type are also configurable.
For constant temperature simulations, a Langevin integrator is commonly used.
If constant pressure is desired, a barostat can be enabled to maintain isotropic
volume fluctuations.

Simulation output includes trajectory files, state checkpoints, and log files
recording thermodynamic properties such as energy, temperature, and pressure.
These outputs can be used to assess convergence, monitor structural drift, or
extract representative frames for further modeling. The user can configure the
frequency with which these outputs are written, balancing file size with the
desired level of detail.

This protocol supports both implicit and explicit solvent models, depending on
how the system was prepared. It also allows the use of constraints and restraints,
which can be applied to specific atoms or groups to stabilize regions of the
molecule or to simulate near-native conformations. The user may also define a
random seed to ensure reproducibility, or allow stochastic components such as
initial velocities to vary between runs.

At the end of the simulation, the user obtains a trajectory compatible with
analysis and visualization protocols in Scipion-Chem. These trajectories can be
further processed for clustering, RMSD analysis, or energetic evaluation, forming
a central part of the molecular modeling pipeline.

In summary, the SystemSimulation protocol provides a flexible and
physics-accurate environment for running molecular dynamics simulations under a
variety of thermodynamic conditions. It serves as the core execution engine for
dynamics-based workflows in Scipion-Chem using the OpenMM backend.
    """
